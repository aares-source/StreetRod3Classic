...
void City_ProcessDriving(void)
{
	...
	cCityCamera.LookAt( cCityCar.getCCarSim()->getSimBody()->getPosition() );
}
...
void City_CheckCollisions(CCar *car, CCity *city)
{
	...
	int x = (int) (car->getCCarSim()->getSimBody()->getPosition().GetX() / tilesize);
	int y = (int) (car->getCCarSim()->getSimBody()->getPosition().GetY() / tilesize);
	int sx = x - 1;
	int sy = y - 1;
	int ex = x + 1;
	int ey = y + 1;
	...
	for(int n=0;n<4;n++)
		car->getCCarSim()->getWheel(n)->getContact()->Touching = false;

	car->getCCarSim()->getContact()->Touching = false;
	...
	for(y=sy; y<=ey; y++) {
		...
		for(x=sx; x<=ex; x++) {
			...
			for(int n=0;n<4;n++) {
				w = car->getCCarSim()->getWheel(n);
				cont = w->getContact();

				// Get the contact ray
				w->getContactRay(car->getCCarSim()->getSimBody()->GetSimBody(w->getRelPos()), &p, &m);
			...
			cont = car->getCCarSim()->getContact();

			car->getCCarSim()->getSimBody()->createDrawMatrix(mat);
			...
			if(carcol->collision( col )) {
				...
				// Get collision point
				carcol->getCollisionPoint(0, v1);
				for(i=0;i<3;i++) cont->Pos.SetIndex(i,v1[i]);

				//cont->Pos = cont->Pos + offset;

				// Get collision normal
				carcol->getCollidingTriangles(0, t1,t2);
