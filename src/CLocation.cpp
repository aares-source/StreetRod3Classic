...
void CLocation::ProcessCamera(void)
{
	...
	// SDL2: no SDL_GetVideoSurface; window size via gSDLWindow if needed
	...
	if(tMouse->Button & SDL_BUTTON(3) && !bCamLocked) {
		...
		if(!iGrabbed) {
			...
			SDL_WarpMouseInWindow(gSDLWindow, (int)vGrabPos.GetX(), (int)vGrabPos.GetY());
			return;
		}
	...
	if(!bCamLocked) {
		if(kb->keys[SDL_SCANCODE_RIGHT])	fcamYaw++;
		if(kb->keys[SDL_SCANCODE_LEFT])		fcamYaw--;
		if(kb->keys[SDL_SCANCODE_UP])		fcamPitch++;
		if(kb->keys[SDL_SCANCODE_DOWN])		fcamPitch--;
	}
