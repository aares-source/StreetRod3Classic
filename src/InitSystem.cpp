...
void System_ProcessInput(void)
{
	...
	// Keyboard
	tKeyboard.keys = SDL_GetKeyboardState(NULL);
...
int System_InitializeOpenGL(void)
{
	...
	gSDLWindow = SDL_CreateWindow("Street Rod 3",
		SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
		800, 600, winFlags);
	if(!gSDLWindow) {
		writeLog("Error: Failed to create window\n");
		writeLog("  SDL Returns:  %s\n", SDL_GetError());
		return false;
	}
	...
	if(!gGLContext) {
		writeLog("Error: Failed to create GL context\n");
		writeLog("  SDL Returns:  %s\n", SDL_GetError());
		return false;
	}
	...
	SDL_SetWindowTitle(gSDLWindow, "Street Rod 3");
    ...
    if( strstr((char *)glGetString( GL_EXTENSIONS ), "GL_ARB_multitexture") != 0 && psOptions->nMultiTextureEnabled ) {
		...
		g_GL_TEXTURE0 = GL_TEXTURE0_ARB;
		g_GL_TEXTURE1 = GL_TEXTURE1_ARB;
        bMultiTexture = true;
    } else {
