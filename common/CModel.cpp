...
///////////////////
// Legacy GetObjects() - maps CModel meshes to a static CObject for compatibility
// NOTE: returns NULL (mesh data is in mesh_t, not CObject). RacingEngine needs updating.
CObject *CModel::GetObjects(void)
{
    // RacingEngine uses CObject* iteration; the data is now in mesh_t.
    // Return NULL - caller must guard against this.
    return NULL;
}
...
bool CModel::loadnew3DData(FILE *fp)
{
    ...
    for( n=0; n<m_nNumMeshes; n++ ) {
        ...
        // Read the polygons
        for( int i=0; i<mesh->nNumPolygons; i++ ) {
