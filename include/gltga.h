...
// Provide types needed if tga.h is absent
#ifndef _TGA_H_
typedef unsigned int  uint;
typedef unsigned char byte;
// Minimal tga_t stub so glTga_t can compile without the original tga.h
struct tga_t {
    unsigned char *data;
    int width, height, bits;
    void *format;
    virtual int Load(const char*) { return 0; }
    virtual int LoadBuffer(byte*, ...) { return 0; }
    virtual void Release() {}
    virtual void Reset() {}
};
// badData for enum below
static const int badData = -1;
#define _TGA_H_
#endif
